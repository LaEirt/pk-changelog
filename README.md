...
- **城市心驿 · 佣金流水对账列**：流水列表/导出补档位、用户手机、归因方式、用户状态；默认每页 100，导出全部给财务自筛。见 `city_station_affiliate_plan.md` T46。
- **矩阵 SCRM · 线索合并切 ODS**：`视频号线索数合并` 改读 `matrix_scrm_*`，可停新手工月表；添加来源/客户描述借微伴或空，客户主体默认「微信」。见 `matrix_scrm_external_user_pull_plan.md` S1。
- **城市心驿 · 佣金流水少过滤**：数据看板流水 Tab 去掉日期/状态/推广官筛，财务主路径为导出全部后再按下单时间调整。见 `city_station_affiliate_plan.md` T45。
