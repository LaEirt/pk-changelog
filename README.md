...
- **工作台 · 咨询交付 · 分配抽屉展示**：顶栏对称展示商品金额/咨询次数（理论）、退款金额/次数（可带退款咨询师）、分配目标与已分/还差；校验池仍为商品金额与有效次数。见 `consult_delivery_plan.md` AMT-15。
- **工作台 · 咨询交付 · 退款后重算分配**：登记/撤销退款次数后主单按交付权重重切「咨询师分配金额」（池仍为商品金额）；修 `CASE_20250723_360095` 次数与金额脱节。见 `consult_delivery_plan.md` AMT-14。
