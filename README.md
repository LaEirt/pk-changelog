...
- **短信推广 · 购后加人跟进**：发送页默认「待跟进」；首屏已发/已打开/已加人/还没加/加人比例；一键复制手机号；运维能力降到「发送与异常」。
...
- **城市心驿站 · 行操作收「更多」**：名单/分组不再外露「重命名」等小按钮；点行进详情，次要操作用 ⋯；详情点姓名改名。见 `city_station_affiliate_plan.md` T28 / T41。
- **城市心驿站 · 老用户计佣可配**：档位增加 `returning_accrue` 开关（种子默认仅 L0 开）；计佣/人工绑不再写死「只有 L0」。见 `city_station_affiliate_plan.md` T20 / §3.4。
- **工作台 · 咨询交付 · 单咨询师退款默认选人**：登记退款次数侧栏，候选仅一人时默认选中并保存自动写入，避免单人单「退款咨询师」空着。见 `consult_delivery_plan.md` REF-CONS-01/02。
- **工作台 · 咨询交付 · 分配含退款份额**：分配次数合计=理论咨询次数；退款计入退款咨询师（例曾 4.5+4.5=9）；金额按权重切；单价分母含退款；主表金额 tag 可拆「咨询+退款」。见 `consult_delivery_plan.md` AMT-14/16。
- **工作台 · 咨询交付 · 分配抽屉展示**：顶栏对称展示商品金额/咨询次数（理论）、退款金额/次数（可带退款咨询师）、分配目标与已分/还差。见 AMT-15。
- **工作台 · 咨询交付 · 导出列序**：本页/全部导出表头与主表一致，含「退款咨询师」「咨询师交付路径」（路径在保护期列前）。见 `consult_delivery_plan.md` EXP-ALIGN。
...
- **公域自动开课 · 视频号补漏**：产品/赠课 match_queue 去重与已开通/已关闭结案；无效手机号结案；退款候选补「快手」；视频号常见课名映射；历史压掉约 3.9 万重复行。见 `xiaoe_delivery_improvement_plan.md` T13/T14 · `sql/ddl/dw_entitlement/`。
- **工作台 · 城市心驿站**：推广官/推广分组名单统一为**点行进入**；操作列均为重命名·启停·删除；分组支持硬删（有佣金拒删）。见 `city_station_affiliate_plan.md` CS-51。
...
- **城市心驿 · 佣金流水筛选**：流水 Tab 恢复并扩展下单日/状态/推广官/平台/归因/档位/用户手机/订单号筛选；导出全部=当前筛条件下全部行。见 `city_station_affiliate_plan.md` T45。
- **城市心驿 · 佣金流水对账列**：流水列表/导出补档位、用户手机、归因方式、用户状态；默认每页 100。见 `city_station_affiliate_plan.md` T46。
- **矩阵 SCRM · 线索合并切 ODS**：`视频号线索数合并` 改读 `matrix_scrm_*`，**只写** `视频号线索数统计`；月表/总表已 DROP（P8.1）；添加来源/客户描述借微伴或空，客户主体默认「微信」。见 `matrix_scrm_external_user_pull_plan.md` S1。
- **矩阵 SCRM · 可替换列对账**：相对 2026-08 月表，企业成员/头像/性别/类型一致；标签维表仅覆盖 ~11% ref（P0，同事命中极低）；群聊偏空；备注/流失可漂移。见 api/index.md §4.1（`index.md`）。
...
- **城市心驿 · 人/组缝合 + 锁客导入**：组内/建组可「用手机号现场加人」；组没人可去名单新建或导入并 `from_pool` 返回该组；人详情芯片进组、未入组可加入。推广官/锁客导入弹窗都有预览表；锁客模板仅「被绑用户手机号 + 绑定码」。见 `city_station_affiliate_plan.md` T36 / T38。
- **城市心驿 · 商城状态文案**：推广官名单「商城」列未绑账号时显示 **未注册**（原「待注册」）。见 `city_station_affiliate_plan.md` T34。

### 2026-09-14

- **城市心驿 · 锁客绑定日期 / 原文 2.5**：工作台锁客填「从哪天算起」，到期日只读、无计佣开关；L0 老用户仍计佣，L1+ 老用户不计；先成交后补绑可按买家手机重跑锁窗（不改已计提）。见 `city_station_affiliate_plan.md` T20 / T37 / T39。
- **城市心驿 · 每商品 L0–Ln 费率 + demo**：规则设置添加商品后立刻配齐当前启用档；引流/进阶是两个商品两张表。`seed_affiliate_demo --wipe` 已按绑定日 + 全档费率重种。见 plan T40。
- **城市心驿 · 锁客筛栏分栏**：买家手机与推广官拆开，并加档位 / 来源 / 计佣 / 状态；详情「去锁客」按推广官 ID 精确带出，不再用混搜框。见 `city_station_affiliate_plan.md` T35。
- **城市心驿 · 钻取 Tab 视觉**：进入分组/推广官后，用带左箭头的 Tab 条返回上级（深色下划线标当前位置），不再用蓝色文字链当引导。见 `city_station_affiliate_plan.md` T29。
- **城市心驿 · 推广官详情 / 邀请落地 ToC**：详情空绑客不再指向已隐藏的「锁客」Tab，改为可点链接切过去并带上手机筛选。商城 `/p/:code` 记下邀请后停在确认页，按未登录 / 未绑手机 / 已就绪给出下一步，不再闪一下就跳走。见 `city_station_affiliate_plan.md` T26 / T29。
- **城市心驿 · 推广官钻取对齐规则设置**：点姓名进入详情时隐藏名单筛选、批量条和 Tab 头，只留面包屑返回；切页带动画。详情补充商城绑定一句说明（已注册未绑定会在对方登录个人中心时自动绑上）。见 `city_station_affiliate_plan.md` T29。
- **城市心驿 · 邀请码自发绑定**：家长打开 `/p/:code` 会记下邀请；登录、注册或绑手机后按买家手机写入邀请留资（此前 cookie 写下后从未消费）。推广官用同手机登录仍自动回填商城账号。见 `city_station_affiliate_plan.md` T26。
- **下载任务 · 大文件超时**：导出任务「下载」原先走全局 axios 15s，销售订单等大 Excel 易超时并误提示「缩小筛选」；下载单独放宽至 180s。见 `workbench_async_export_plan.md`。
- **shop 库整理**：DROP 已切源遗留 `product_info.xiaoe_product_*`（约 39MB）与空壳 `public.用户信息`；`user_info` 商城/城市心驿表全部保留。脚本 `scripts/db/cleanup_shop_unused_tables.py`。
- **商城 · 登录态绑邮箱**：个人中心邮箱行新增「去绑定 / 换绑」，邮件 6 位验证码（冷却 60s）；手机号 OTP 进站的空邮箱账号可补绑邮箱用于密码登录/找回。见 `storefront_toc_benchmark_plan.md` TB-T-DLV-10。
- **城市心驿 · 邀请码不可见修复**：运营按手机号建档/导入的推广官 `user_id` 为空，商城个人中心看不到「我的推广」；现登录后按手机号自动匹配回填（审计 `promoter_auto_bind`，已绑他人不抢占）。见 `city_station_affiliate_plan.md` T26。
- **城市心驿 · 运营台绑商城账号**：推广官详情可按手机号绑定/解绑商城买家；名单显示 **待注册 / 已注册 / 已绑定** 与商城用户名；新建/导入时若手机号已注册则自动填。见 `city_station_affiliate_plan.md` T34。
- **城市心驿 · 数据看板可视化**：概览工具条改为近 7 天/本月/近 30 天快捷档；KPI 卡对齐个人中心经营分析；图表改用 ECharts（日佣金趋势、Top 条形、平台/档位/锁客来源环形）。见 `city_station_affiliate_plan.md` FR-10。

### 2026-09-11

- **城市心驿 · 推广官行内停用/删除 + 分页**：名单操作列可停用、删除（有佣金或有效锁客不能删）；**各档位表格内翻页**，不是整页一条分页。见 `city_station_affiliate_plan.md` T33。
...
- **城市心驿 · 运营台补齐**：花名册多选入推广池、默认费率矩阵页、佣金流水改归因、邀请码停用；计提单测 CS-01/03/06–09 green。见 `city_station_affiliate_plan.md`。
...
- **工作台 · 经营分析磁贴自动避让**：拖移/缩放时重叠组件自动下推，松手垂直收紧；工具栏「整理布局」；新增/粘贴走空位探测。见 `bi_dashboard_authoring.md`。
- **工作台 · 经营分析右栏 Soft TOC（BI-TOC-06）**：字段/样式/分析三 Tab 改为胶囊分段；各 Tab 顶部 Soft 分区芯片可跳转；样式与分析共用 Soft 折叠。见 `bi_toc_ux.md`。
- **工作台 · 经营分析 T5/T6 字段与数据区**：字段芯片齿轮菜单（聚合/累计/数字格式/空值/日期格式）；批量改格式；空值与日期展示接到图表渲染；制作台左栏「预览样例数据」。见 `bi_dashboard_improvement.md`。
- **工作台 · 经营分析样式面板丰富（DASH-I-32）**：样式 Tab 改为折叠分区（标题卡片/配色色板/柱宽圆角/图例对齐/数据标签/坐标轴/提示/数字格式），写入 `style` 并由 `biApplyChartStyle` 落到 ECharts。见 `bi_dashboard_improvement.md`。
- **工作台 · 经营分析制作台 Soft UX**：图表绝对铺满磁贴（消滚动条空带）；字段架去掉说明书腔、分区卡片与底部「更新看数」条；布局条改为浮层。见 `bi_dashboard_authoring.md`。
- **工作台 · 经营分析仪表专项 P2 余项**：度量累计/按年累计；交叉表占比；图级过滤算子 + 预览行数；多选批量粘贴样式；页面设置（水印/下载/记住查询）与页主题/板备注/页尾。见 `bi_dashboard_improvement.md`。
...
- **工作台 · 数据表→制作全流程**：数据表页可抽样预览；一键「用此表新建看板」；新建接口可绑 `dataset_id`。见 `workbench_bi_plan.md` FR-P3-01 / G21。
- **工作台 · 经营分析时间轴**：编辑器可加「时间轴」（按月节点）；点节点可联动筛选。见 `workbench_bi_plan.md` §4.3 P3。
- **工作台 · 经营分析词云**：编辑器可加「词云」（分类字号按金额）；点词可联动筛选。见 `workbench_bi_plan.md` §4.3 P3。
- **工作台 · 经营分析散点图**：编辑器可加「散点图」（分类 + 双度量 XY）；点可联动筛选；可导出 PNG。见 `workbench_bi_plan.md` §4.3 P3。
- **工作台 · 经营分析雷达图**：编辑器可加「雷达图」（分类作轴）；下方轴标签可点选联动；可导出 PNG。见 `workbench_bi_plan.md` §4.3 P3。
- **工作台 · 经营分析漏斗图**：编辑器可加「漏斗图」（分类阶段×金额）；点阶段可联动筛选；可导出 PNG。见 `workbench_bi_plan.md` §4.3 P3。
- **工作台 · 经营分析色块热力**：编辑器可加「色块热力」（月×分类）；点色块可按行维联动；可导出 PNG。见 `workbench_bi_plan.md` §4.6.9。
- **工作台 · 经营分析仪表图组件**：编辑器可加「仪表图」；目标值为满刻度，可导出 PNG。见 `workbench_bi_plan.md` FR-P2-01。
- **工作台 · 经营分析进度条组件**：编辑器可加「进度条」；可配目标值，点分类可联动筛选。见 `workbench_bi_plan.md` §4.6 / FR-P2-01。
- **工作台 · 经营分析查询审计**：制作台可看查询耗时 P50/P95 与缓存命中；门户列表收藏优先置顶。见 `workbench_bi_plan.md` L-10 / FR-P3-04。
- **工作台 · 经营分析制作台软删/重命名/排序**：自建板可软删；可重命名；列表可按修改时间或标题排序。系统种子板不可删。见 `workbench_bi_plan.md` §7.5。
- **工作台 · 经营分析制作台复制仪表板**：可将已有看板复制为新草稿再改；编辑器可恢复已隐藏组件。见 `workbench_bi_plan.md` API-BI-06b。
- **工作台 · 经营分析门户仅收藏 + 单图 PNG**：门户可筛「仅收藏」；预览柱/线/饼等组件菜单可导出 PNG。见 `workbench_bi_plan.md` FR-P2-03 / 门户。
- **工作台 · 经营分析数据源探活**：制作台左栏「数据源」可看平台已接连接与延迟；不展示口令。制作台「仅我的」按所有者筛选。见 `workbench_bi_plan.md` §3.8 L-02。
- **工作台 · 经营分析导出 Excel**：组件菜单可导出 `.xlsx`（默认）或 CSV（UTF-8 BOM）；同步导出上限仍 5000 行。见 `workbench_bi_plan.md` FR-P2-03。
- **工作台 · 经营分析交叉表联动 + 组件剪贴板**：预览点交叉表行可按行维筛选；编辑器支持「复制到剪贴板 / 粘贴组件」跨板复用配置（非服务端 chart 实体）。见 `workbench_bi_plan.md` FR-P2-02/04。
- **工作台 · 经营分析同环比展示**：分析 Tab 开环比/同比后，KPI 卡显示差值色；折线/组合图增加虚线「上期」系列。见 `workbench_bi_plan.md` FR-P2-05。
- **工作台 · 经营分析查询调度（GAP-07/08）**：预览同屏查询并发上限 4、改筛取消排队中旧请求；编辑器连点「更新看数」会 Abort 上次查询。见 `workbench_bi_plan.md` §4.7。
- **工作台 · 经营分析百分比堆积柱**：堆积图可开「百分比堆积」（合计=100%）；全 IP「01b」金额卡默认占比；旧板自动升级。见 `workbench_bi_plan.md` §4.6.8。
