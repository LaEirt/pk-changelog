...
- **工作台 · 经营分析 P0–P3**：门户 + Studio/草稿发布 + 编辑器骨架 + 排行/CSV 导出 + 集中授权/行列策略/诊断；数据源 `日报.全域订单`；权限 `bi_view`/`bi_edit`/`bi_admin`/`bi_dataset_manage`。P4（外部 Quick BI 切流）未做。见 `workbench_bi_plan.md`。
