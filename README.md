...
- **工作台 · 经营分析建板引导 / 数字格式**：编辑器首访 3 步 tour（加图→配字段→保存发布）+ 顶栏步骤条；样式页数字格式（智能万/强制万/整数/原值）；助手知识库改为侧栏门户/Studio。见 `workbench_bi_plan.md` FR-P1-10。
- **工作台 · 经营分析 BI 自助深化**：样式标题/色、布局疏密、预览水印用户名、查询控件配置、Studio 轻量文件夹、编辑态查看数据。见 `workbench_bi_plan.md` §3.7。
